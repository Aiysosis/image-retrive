...
def get_top_k_similar(image_data, pred, pred_final, k):
    ...
    print('pred', len(pred));
    top_k_ind = [cosine(image_data, pred_row) \
                 for ith_row, pred_row in enumerate(pred)][:2955]

    top_k_ind = np.argsort(top_k_ind)
...
def recommend(imagePath, extracted_features):
    ...
    return get_top_k_similar(features, extracted_features, neighbor_list, k=80)
